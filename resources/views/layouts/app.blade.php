        ...
        <div class="wrap">
            @php
                $breadcrumbs = match (true) {
                    ...
                    request()->routeIs('dermaga.*') => [['label'=>'Dermaga','url'=>null], ['label'=>match (true) {
                                request()->routeIs('dermaga.control-center') => 'Control Center',
                                request()->routeIs('dermaga.checkin') => 'Check-in / Scan',
                                request()->routeIs('dermaga.boarding*') => 'Boarding',
                                request()->routeIs('dermaga.dispatch*') => 'Dispatch',
                                default => 'Manifest',
                            },'url'=>null]],
                    request()->routeIs('ticketing.*') => [['label'=>'Operasional','url'=>null],['label'=>'Ticketing / Kasir','url'=>null]],
                    request()->routeIs('operasional.*') => [['label'=>'Operasional','url'=>null],['label'=>match (true) {
                                request()->routeIs('operasional.pooling*') => 'Pooling',
                                request()->routeIs('operasional.queue*') => 'Antrian Boat',
                                default => 'Exception',
                            },'url'=>null]],
                    request()->routeIs('master.*') => [['label'=>'Master Data','url'=>null],['label'=>match (true) {
                                request()->routeIs('master.boats.*') => 'Boat',
                                request()->routeIs('master.customers.*') => 'Customer',
                                request()->routeIs('master.agents.*') => 'Agent',
                                request()->routeIs('master.schedules.*') => 'Trip / Jadwal',
                                default => 'Tarif',
                            },'url'=>null]],
                    default => [],
                };
